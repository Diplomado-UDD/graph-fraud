...
def __(mo, transactions_df, user_devices_df, users_df):
    ...
    # Users schema
    expected_user_cols = ['user_id', 'is_fraudster', 'account_age_days', 'verification_level']
    users_check = all(col in users_df.columns for col in expected_user_cols)
    validation_checks.append({
        'Check': 'Users Table Schema',
        'Status': 'Pass' if users_check else 'Fail'
    })

    # Transactions schema
    expected_txn_cols = ['transaction_id', 'sender_id', 'receiver_id', 'amount', 'timestamp', 'is_fraudulent']
    ...
    ref_check = sender_ids.issubset(all_user_ids) and receiver_ids.issubset(all_user_ids)
    validation_checks.append({
    ...
    validation_checks.append({
        'Check': 'Account Ages Positive',
        'Status': 'Pass' if age_check else 'Fail'
    })
...
def __(mo, plt, sns, users_df):
    ...
    sns.histplot(data=users_df, x='account_age_days', hue='is_fraudster',
    ...
    fraud_counts = users_df.groupby(['verification_level', 'is_fraudster']).size().unstack()
...
def __(mo, plt, sns, transactions_df):
    ...
    sns.histplot(data=transactions_df, x='amount', hue='is_fraudulent',
                 kde=True, ax=axes_txn[0, 0], bins=50)
    ...
    status_counts = transactions_df.groupby(['status', 'is_fraudulent']).size().unstack()
...
def __(mo, pd, transactions_df, users_df):
    ...
    txn_stats_by_user = transactions_df.groupby('sender_id').agg({
    ...
    txn_stats_with_fraud = txn_stats_by_user.merge(
        users_df[['user_id', 'is_fraudster']],
        on='user_id',
        how='left'
    )

    txn_summary = txn_stats_with_fraud.groupby('is_fraudster')[['txn_count', 'avg_amount', 'total_amount']].describe()
...
def __(devices_df, mo, pd, user_devices_df, users_df):
    ...
    device_sharing = user_devices_df.groupby('device_id')['user_id'].agg(['count', list]).reset_index()
...
def __(mo, np, plt, sns, transactions_df, users_df):
    ...
    txn_features = transactions_df.groupby('sender_id').agg({
        'amount': ['count', 'mean', 'sum'],
        'is_fraudulent': 'sum'
    }).reset_index()
    txn_features.columns = ['user_id', 'txn_count', 'avg_txn_amount', 'total_txn_amount', 'fraud_txn_count']

    # Merge features
    feature_df = user_features.merge(txn_features, on='user_id', how='left').fillna(0)

    # Calculate correlation matrix
    corr_cols = ['is_fraudster_int', 'account_age_days', 'txn_count', 'avg_txn_amount', 'total_txn_amount', 'fraud_txn_count']
    corr_matrix = feature_df[corr_cols].corr()

    # Plot correlation heatmap
    fig_corr, ax_corr = plt.subplots(figsize=(10, 8))
    sns.heatmap(corr_matrix, annot=True, fmt='.2f', cmap='coolwarm', center=0,
                square=True, ax=ax_corr, vmin=-1, vmax=1)

...
def __(G, mo, np, plt):
    ...
    user_degrees = [G.degree(n) for n in G.nodes() if G.nodes[n].get('node_type') == 'user']
    device_degrees = [G.degree(n) for n in G.nodes() if G.nodes[n].get('node_type') == 'device']
    ...
    axes_deg[0].axvline(np.mean(degrees), color='red', linestyle='--', label=f'Mean: {np.mean(degrees):.2f}')
    axes_deg[0].legend()

    # User degree distribution
    axes_deg[1].hist(user_degrees, bins=20, edgecolor='black', alpha=0.7, color='skyblue')
    axes_deg[1].set_title('User Node Degree Distribution')
    axes_deg[1].set_xlabel('Degree')
    axes_deg[1].set_ylabel('Count')
    axes_deg[1].axvline(np.mean(user_degrees), color='red', linestyle='--', label=f'Mean: {np.mean(user_degrees):.2f}')
    axes_deg[1].legend()

    # Device degree distribution
    axes_deg[2].hist(device_degrees, bins=15, edgecolor='black', alpha=0.7, color='lightcoral')
    axes_deg[2].set_title('Device Node Degree Distribution')
    axes_deg[2].set_xlabel('Degree')
    axes_deg[2].set_ylabel('Count')
    axes_deg[2].axvline(np.mean(device_degrees), color='red', linestyle='--', label=f'Mean: {np.mean(device_degrees):.2f}')
    axes_deg[2].legend()
...
def __(communities, dataset, mo, pd, plt):
    ...
    for comm_id in set(communities.values()):
        ...
        community_fraud_analysis.append({
            ...
            'Fraud Rate': fraud_count / len(comm_members) if len(comm_members) > 0 else 0
        })

    comm_fraud_df = pd.DataFrame(community_fraud_analysis).sort_values('Fraud Rate', ascending=False)

    # Plot fraud rate by community
    fig_comm, ax_comm = plt.subplots(figsize=(12, 6))
    ax_comm.bar(comm_fraud_df['Community ID'], comm_fraud_df['Fraud Rate'], color='coral')
    ax_comm.set_xlabel('Community ID')
    ax_comm.set_ylabel('Fraud Rate')
    ax_comm.set_title('Fraud Rate by Community')
    ax_comm.axhline(y=0.15, color='red', linestyle='--', label='Overall Fraud Rate (15%)')
    ax_comm.legend()
    plt.tight_layout()
...
def __(detector, mo, pd):
    ...
    top_pagerank = centrality_df.nlargest(10, 'pagerank')[['user_id', 'pagerank', 'is_fraudster']]
    top_betweenness = centrality_df.nlargest(10, 'betweenness')[['user_id', 'betweenness', 'is_fraudster']]
...
def __(fraud_graph, mo, pd):
    ...
    for device_id, user_list in shared_devices.items():
        ...
        shared_analysis.append({
            ...
            'Users': ', '.join(user_list[:3]) + ('...' if len(user_list) > 3 else '')
        })
...
def __(mo, nx, plt, transaction_network):
    ...
    axes_txn_net[0].hist(list(in_degrees.values()), bins=20, edgecolor='black', alpha=0.7, color='lightblue')
    ...
    axes_txn_net[1].hist(list(out_degrees.values()), bins=20, edgecolor='black', alpha=0.7, color='lightgreen')
...
def __(G, dataset, mo, nx, plt):
    ...
    fraud_nodes = [n for n in G.nodes() if n in fraudsters or
                   (G.nodes[n].get('node_type') == 'device' and
                    any(neighbor in fraudsters for neighbor in G.neighbors(n)))]
    ...
    nx.draw_networkx_nodes(fraud_subgraph, pos, node_color=node_colors,
                           node_size=300, alpha=0.8, ax=ax_fraud_net)
    ...
    ax_fraud_net.set_title('Fraud Ring Network (Red = Fraudsters, Blue = Shared Devices)', fontsize=14)

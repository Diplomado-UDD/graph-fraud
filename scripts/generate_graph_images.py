...
def draw_overview(G, path=None):
    ...
    nx.draw_networkx_nodes(sub, pos, nodelist=user_nodes, node_color='#2b8cbe', node_size=80, label='users')
    nx.draw_networkx_nodes(sub, pos, nodelist=device_nodes, node_color='#7fc97f', node_size=60, label='devices')
    # draw edges lightly
...
def draw_transaction_path(G, risk_df):
    ...
    nx.draw_networkx_labels(sub, pos, {n: sub.nodes[n].get('label','') for n in sub.nodes()}, font_size=8)
    plt.axis('off')

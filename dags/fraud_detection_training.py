...
def generate_dataset(**context):
    ...
    print(f"Dataset generated: {len(dataset['users'])} users, {len(dataset['transactions'])} transactions")
...
def train_model(**context):
    ...
    with mlflow.start_run(run_name=f"airflow_training_{datetime.now().strftime('%Y%m%d_%H%M%S')}"):
        ...
        # Check criteria
        meets_criteria = (
            precision >= config.TARGET_PRECISION_MIN and
            precision <= config.TARGET_PRECISION_MAX and
            recall >= config.TARGET_RECALL_MIN
        )

        mlflow.log_param("meets_criteria", meets_criteria)

        print(f"Model trained - Precision: {precision:.3f}, Recall: {recall:.3f}, F1: {f1_score:.3f}")
...
# Set dependencies
generate_data_task >> validate_data_task >> track_with_dvc_task >> build_graph_task >> train_model_task

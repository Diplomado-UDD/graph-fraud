...
class FraudDetector:
    ...
    def calculate_centrality_scores(self) -> pd.DataFrame:
        ...
        user_nodes = [
            n
            for n in self.transaction_network.nodes()
            ...
            scores.append(
                {
                    ...
                    "is_fraudster": self.transaction_network.nodes[node][
                        "is_fraudster"
                    ],
                }
            )
    ...
    def detect_shared_resources(self) -> List[Dict]:
        ...
        for device_id, user_list in shared_devices.items():
            fraud_count = sum(
                1 for u in user_list if self.G.nodes[u].get("is_fraudster", False)
            )
    ...
    def calculate_transaction_velocity(
        self, transactions_df: pd.DataFrame, window_hours: int = 24
    ) -> pd.DataFrame:
        ...
        for user_id in transactions_df["sender_id"].unique():
            user_txns = transactions_df[
                transactions_df["sender_id"] == user_id
    ...
    def compute_risk_scores(
        ...
        for resource in shared_resources:
            for user_id in resource["shared_by"]:
                device_risk[user_id] = max(
                    device_risk.get(user_id, 0), resource["risk_score"]
                )
        ...
        if transactions_df is not None:
            ...
            risk_df["avg_txn_amount"] = (
                risk_df["user_id"].map(user_avg_amounts).fillna(0)
            )
            # Normalize: amounts > $500 are suspicious (fraud avg ~$2493, normal avg ~$247)
            risk_df["amount_risk"] = (risk_df["avg_txn_amount"] - 500).clip(
                lower=0
            ) / 2500

            # Transaction volume risk
            user_txn_counts = (
                transactions_df[transactions_df["sender_id"].isin(risk_df["user_id"])]
                .groupby("sender_id")
                .size()
            )
    ...
    def generate_fraud_report(
        self, transactions_df: pd.DataFrame, risk_threshold: float = 0.15
    ) -> Dict:
        ...
        risk_df = self.compute_risk_scores(
            centrality_df, shared_resources, transactions_df
        )
        ...
        if not community_df.empty:
            community_df = community_df.merge(
                risk_df[["user_id", "is_fraudster"]], on="user_id", how="left"
            )
            ...
            community_stats["fraud_rate"] = (
                community_stats["fraudster_count"] / community_stats["size"]
            )
        else:

...
class FraudDatasetGenerator:
    ...
    def generate_users(self, n_users: int = 200, fraud_rate: float = 0.15) -> pd.DataFrame:
        ...
        for i in range(n_users):
            users.append({
                ...
                "account_age_days": random.randint(1, 1000) if i >= n_fraudsters else random.randint(1, 90),
                "verification_level": random.choice(["basic", "verified", "premium"]) if i >= n_fraudsters else "basic",
            })

        return pd.DataFrame(users)

    def generate_fraud_rings(self, users_df: pd.DataFrame, n_rings: int = 3) -> List[List[str]]:
    ...
    def generate_devices(self, users_df: pd.DataFrame) -> pd.DataFrame:
        ...
        other_fraudsters = [
            uid for uid in users_df[users_df["is_fraudster"]]["user_id"].tolist()
    ...
    def generate_dataset(self, n_users: int = 200, n_transactions: int = 1000) -> Dict[str, pd.DataFrame]:
    ...
    def save_dataset(self, dataset: Dict[str, pd.DataFrame], output_dir: str = "data/raw"):
